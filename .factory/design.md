...
`site/public/query-fern.webp` is the hero specimen: a pressed fern whose leaflets subtly resolve into orderly database rows, accompanied by a blank accession tag and a small red approval thread. It explains the product's “classify, bound, record” model while leaving page copy to semantic HTML.
